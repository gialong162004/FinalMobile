...
public interface ProductRepository extends JpaRepository<Product,Integer>{
	...
	@Query(value="select * from `fashionstore`.product where `fashionstore`.product.product_name like CONCAT('%', ?1, '%') and `fashionstore`.product.category_id= ?2",nativeQuery = true)
